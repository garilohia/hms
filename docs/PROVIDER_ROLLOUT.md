...
- HTTP 429 responses persist a connection-scoped cooldown. Manual sync, token refresh and further pages respect it; another successful request cannot shorten it. `Retry-After` supports seconds and HTTP dates, with WHOOP reset seconds as a fallback. This is not a shared client-quota budget. Public rollout still needs aggregate quota enforcement and source-to-recipient load/device acceptance before any minute-level promise.
