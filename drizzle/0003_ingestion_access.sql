-- Only checked ingestion functions can create sources/metrics/jobs.
REVOKE ALL ON public.summary_jobs FROM PUBLIC, anon, authenticated;

CREATE FUNCTION hms_private.connect_source(p_subject uuid, p_provider public.provider, p_key text) RETURNS uuid
...
BEGIN
  ...
  IF p_provider IS NULL OR p_provider NOT IN ('simulator','generic_csv','apple_health_export')
    OR p_key IS NULL OR length(trim(p_key)) NOT BETWEEN 1 AND 200 THEN
  ...
  INSERT INTO public.data_sources(user_id,provider,source_key,metadata)
    VALUES (p_subject,p_provider,'root:'||trim(p_key),jsonb_build_object('label',trim(p_key),'sample',p_provider='simulator'))
    ON CONFLICT (user_id,provider,source_key) DO UPDATE SET status='connected'
  ...
  IF NOT hms_private.is_self(p_subject) THEN
    INSERT INTO public.audit_log(actor_id,action,target_user_id,target_table,target_id)
      VALUES(auth.uid(),'guardian_source_connect',p_subject,'data_sources',result);
  END IF;
  RETURN result;
END; $fn$;

CREATE FUNCTION hms_private.ingest_batch(p_subject uuid, p_source uuid, p_metrics jsonb) RETURNS jsonb
LANGUAGE plpgsql SECURITY DEFINER SET search_path = '' AS $fn$
...
BEGIN
  ...
  WITH added AS (
    INSERT INTO public.metrics(user_id,source_id,metric_type,value,unit,recorded_at,duration_s,quality,external_id)
      SELECT p_subject,coalesce(d.id,p_source),m.metric_type,m.value,m.unit,m.recorded_at,m.duration_s,m.quality,m.external_id
      FROM jsonb_to_recordset(p_metrics) AS m(metric_type public.metric_type,value numeric,unit text,recorded_at timestamptz,duration_s integer,quality public.metric_quality,external_id text,device text)
...
CREATE FUNCTION public.hms_connect_source(p_subject uuid,p_provider public.provider,p_key text) RETURNS uuid
LANGUAGE sql SECURITY INVOKER SET search_path = '' AS $fn$ SELECT hms_private.connect_source(p_subject,p_provider,p_key); $fn$;
CREATE FUNCTION public.hms_ingest_batch(p_subject uuid,p_source uuid,p_metrics jsonb) RETURNS jsonb
LANGUAGE sql SECURITY INVOKER SET search_path = '' AS $fn$ SELECT hms_private.ingest_batch(p_subject,p_source,p_metrics); $fn$;
REVOKE ALL ON FUNCTION hms_private.connect_source(uuid,public.provider,text), hms_private.ingest_batch(uuid,uuid,jsonb),
  public.hms_connect_source(uuid,public.provider,text), public.hms_ingest_batch(uuid,uuid,jsonb) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION hms_private.connect_source(uuid,public.provider,text), hms_private.ingest_batch(uuid,uuid,jsonb),
  public.hms_connect_source(uuid,public.provider,text), public.hms_ingest_batch(uuid,uuid,jsonb) TO authenticated;
