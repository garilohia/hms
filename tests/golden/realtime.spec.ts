...
test("Today, History and consultation chat update without refresh",async({page,browser,baseURL})=>{
  ...
  try{
    ...
    await f.db.unsafe("insert into public.daily_summaries(user_id,day,rhr,hrv_avg,spo2_avg,sleep_duration_min,steps,contains_sample,source_ids,recovery_evidence) select $1,current_date-n,60,40,97,420,5000,true,jsonb_build_object('rhr',$2::text),'{}'::jsonb from generate_series(0,29) n",[patient.subject,source.id]);
