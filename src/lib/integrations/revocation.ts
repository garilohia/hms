...
export async function revokeProviderAccess(provider: IntegrationProvider, current: StoredTokens): Promise<RevocationResult> {
  ...
  try {
    if (provider === "google_health") {
      ...
      const response = await providerRequest(provider, "https://oauth2.googleapis.com/revoke", {
    ...
    const revoke = () => providerRequest(provider, "https://api.prod.whoop.com/developer/v2/user/access", {
    ...
    if (response.status === 401 && tokens.refreshToken) {
      ...
      const refreshed = await providerRequest(provider, config.tokenEndpoint, {
