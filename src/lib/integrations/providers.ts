...
import { providerRequest } from "./request-budget";
...
export async function exchangeCode(provider: IntegrationProvider, code: string, redirectUri: string, verifier?:string, nonce?:string): Promise<{tokens: StoredTokens; externalId: string; scopes: string[]}> {
  ...
  const response = await providerRequest(provider, config.tokenEndpoint, { method: "POST", headers: { "Content-Type": "application/x-www-form-urlencoded", Accept: "application/json" }, body, cache: "no-store", redirect: "error", signal }, deadline);
  ...
  if (provider === "google_health") {
    ...
    const profile = await providerRequest(provider, "https://api.prod.whoop.com/developer/v2/user/profile/basic", { headers: { Authorization: `Bearer ${parsed.data.access_token}` }, cache: "no-store", redirect: "error", signal }, deadline);
...
export async function refreshAccessToken(provider: IntegrationProvider, current: StoredTokens, deadline = Date.now() + 20_000): Promise<StoredTokens> {
  ...
  const response = await providerRequest(provider, config.tokenEndpoint, { method: "POST", headers: { "Content-Type": "application/x-www-form-urlencoded", Accept: "application/json" }, body, cache: "no-store", redirect: "error", signal: AbortSignal.timeout(remaining) }, deadline);
