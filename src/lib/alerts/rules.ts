...
export const defaultRules: AlertRule[] = [
  ...
  ["temp-shift", "skin_temperature", "gt", "absolute", 1, 7200, "attention"],
...
export const metricLabels: Partial<Record<MetricType, string>> = { spo2: "SpO2", heart_rate: "heart rate", resting_heart_rate: "resting heart rate", skin_temperature: "skin temperature", blood_pressure_systolic: "systolic blood pressure", blood_pressure_diastolic: "diastolic blood pressure" };
...
export function evaluateAlerts(metrics: readonly AlertMetric[], baselines: BaselineSet, rules: readonly AlertRule[], options: { day: string; timezone: string; now: Date }) {
  ...
  for (const rule of rules.filter(r => r.enabled)) {
    ...
    const temperature = rule.rule_key === "temp-shift";
    if ((temperature || rule.threshold_type === "baseline_deviation") && (!baseline || baseline.n < 7 || baseline.median === null)) continue;
    ...
    for (const [sourceId, rows] of sources) {
      const series = rows.map(m => ({ recorded_at: m.recorded_at, value: temperature ? m.value - baseline!.median! : m.value,
      ...
      for (const event of detectAnomalies(series, baseline ?? null, { ...rule, require_rest: ["heart_rate", "resting_heart_rate"].includes(rule.metric_type) })) {
        if (Date.parse(event.end) < bounds.start || Date.parse(event.start) >= until) continue;
        const peak = event.peak + (temperature ? baseline!.median! : 0);
        ...
        events.push({ ...event, peak, peakAt, deviation_mads: temperature ? baseline?.mad ? Math.abs(event.peak) / baseline.mad : null : event.deviation_mads,
