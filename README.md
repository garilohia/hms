...
Folder imports accept up to 20,000 CSV files and stream them sequentially. They auto-detect the HMS four-column format and Google Fit `Daily activity metrics` fields for steps, total calories, average heart rate and average weight. When a directory contains Google's consolidated `Daily activity metrics.csv`, redundant date-named CSVs beside it are skipped to avoid double-counting. Other CSV layouts are reported and skipped; diagnostics name the first 20. JSON/TCX files and other Google Takeout datasets are not silently reinterpreted. Date-only Google rows use the selected profile's history timezone. Re-select the same folder with the same source name for an idempotent retry.
