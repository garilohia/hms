...
The same page has a wearable connection hub. Google Health covers Fitbit Air, Fitbit, Pixel Watch and supported Google Health devices; WHOOP has its own read-only OAuth connection. Set the matching client ID/secret variables and a 32-byte base64 `INTEGRATION_TOKEN_KEY`, then register `/api/integrations/google_health/callback` and `/api/integrations/whoop/callback` at each provider. The first link and each manual sync pull the latest seven days through the normal ingestion boundary. Provider tokens are encrypted in the private database schema. Apple Watch uses the Apple Health ZIP flow because HealthKit has no browser OAuth. Garmin and Ultrahuman are labelled partner-gated until the required vendor agreements exist; other devices can use Apple Health, Google Health/Takeout or a supported CSV export.
