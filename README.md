...
Native companion implementation and release criteria are documented in [`mobile/README.md`](mobile/README.md), [`docs/NATIVE_ACCEPTANCE.md`](docs/NATIVE_ACCEPTANCE.md), [`docs/PROVIDER_ROLLOUT.md`](docs/PROVIDER_ROLLOUT.md) and [`docs/LAUNCH_GATES.md`](docs/LAUNCH_GATES.md). These are testable contracts and launch gates, not a claim that unsigned iOS/Android binaries or vendor partnerships already exist.
...
Consults attach immutable summaries. The doctor accepts, can schedule a future time and paste an HTTPS Google Meet/Zoom link, exchanges messages, then closes with a note that appears in History. Today, History and chat subscribe to private Realtime change notifications and re-read their authorised views. Chat keeps Refresh messages as a recovery control. No video service or payment is provisioned.
...
`/more/alerts` manages owner/guardian defaults, monitor-authored thresholds, acknowledgement, notification readiness, recent delivery state, contact consent and per-browser Web Push. A doctor or caregiver needs current alert scope; their rules are retired immediately when access is withdrawn. Generate a VAPID key pair and configure its public/private keys plus subject to enable server push; push payloads contain only an HMS deep link, never a health value. SMS and WhatsApp remain explicit stubs. No real email is sent without configured Resend credentials/sender; sample notifications always use privacy-preserving stubs. Never rely on these notices for emergencies.
...
The same command seeds the fictional doctor directory and 25 dated device catalogue entries. `npm run seed:devices` updates only those named entries and verifies their public RLS read; it preserves unrelated catalogue rows. `/more/devices` compares source-verified features with currency/budget, battery, screen and delivery-path filters. Unknown prices remain “Check price”; no currency conversion or automatic device integration is implied. Update the row's sources and verification date whenever changing a price/specification. The live catalogue UI test is in `npm run golden:verify`, not the credential-free CI suite.
...
The same page has a wearable connection hub covering watches, bands, rings, smart scales, blood-pressure monitors, CGMs and fitness platforms. Google Health and WHOOP have read-only OAuth connections today. Set their client ID/secret variables and a 32-byte base64 `INTEGRATION_TOKEN_KEY`, then register `/api/integrations/google_health/callback` and `/api/integrations/whoop/callback`. Connected accounts are polled by the minute fallback; fresh batches take an inline summary/alert fast path, with cron retained as durable retry. Provider tokens are encrypted in the private schema. Apple Health and Android Health Connect cannot be read by a website; their continuous paths require native companions, while current ZIP/Takeout import covers portable history. Garmin, Oura, Withings and clinical feeds are visibly partner-gated until their agreements and provider-specific webhooks exist. The UI separates live streams, near-real-time cloud events, periodic derived metrics and manual imports. Metrics retain both device recording time and HMS receipt time so actual latency can replace estimates once integrations run in production.

The native-server protocol is available under `/api/mobile/*`: bearer-authenticated bootstrap, stable HealthKit/Health Connect source registration and immutable idempotent batches. It uses the same consent, ownership, canonical validation and summary/alert path as browser imports. `/more/advanced/latency` shows audited median/p95 source, alert and delivery timing plus feed freshness. The platform implementation and physical-device acceptance gates are documented in `mobile/`; no signed native binary is claimed by this repository.
...
The linked Vercel project has a production site at [hms-indol-psi.vercel.app](https://hms-indol-psi.vercel.app), and hosted CI has passed both quality and database replay. M7's build-only fallback in D017 is historical evidence. Preview still needs deliberate Supabase configuration and synthetic data (OQ011). Confirm production Auth callbacks and inbox delivery, activate and observe the minute scheduler, and verify email and physical-device push before claiming those services work. Legal and clinical launch gates remain in OPEN_QUESTIONS.md and docs/LAUNCH_GATES.md. A local build or test pass does not establish which commit is currently deployed.
