...
export function ClinicalSummary({initial,owner}:{initial:z.infer<typeof clinicalSnapshotSchema>;owner:boolean}) {
  ...
  // Full navigations show HTML before listeners attach. Do not accept an inert click or input.
  const hydrated=useSyncExternalStore(subscribeHydration,hydratedSnapshot,serverSnapshot);
  const controlsDisabled=!hydrated||busy;
  ...
  return <div className="stack"><section className="card stack"><h2>{body.profile.name}</h2><p>DOB {body.profile.dob}. {body.profile.sex_at_birth||"Sex not recorded"}</p>
    ...
    <p className="muted">{body.from} to {body.to}, {body.profile.timezone}</p><div className="chips">{([30,90] as const).map(days=><button key={days} className="chip" aria-pressed={body.days===days} disabled={controlsDisabled} onClick={()=>refresh(days)}>{days} days</button>)}</div>
    {owner&&<button className="button secondary" disabled={controlsDisabled} onClick={()=>refresh(body.days,true)}>Save current summary snapshot</button>}
  ...
  <section className="card stack"><h2>Reported medications</h2>{body.medications.length?<ul>{body.medications.map((m,i)=><li key={i}>{m}</li>)}</ul>:<p className="muted">Not recorded. This does not mean none are taken.</p>}
    {owner&&<><label>Your current list (one per line, up to 12)<textarea disabled={controlsDisabled} value={medications} onChange={e=>setMedications(e.target.value)} maxLength={1452}/></label><p className="muted">Report what you currently take. This is not a recommendation. Processing consent must be on in Devices &amp; data.</p><button className="button secondary" disabled={controlsDisabled} onClick={saveMedications}>Save reported medications</button></>}
