...
export default async function LatencyPage({ searchParams }: { searchParams: Promise<{ profile?: string }> }) {
  ...
  return <AppFrame profile={view.profile} profiles={profiles}><div className="stack"><div><h1 className="page-title">Data freshness and latency</h1><p className="muted">Measured delivery timing for {view.profile.name}. Estimates are never presented as measurements.</p></div>
    <section className="card stack"><h2 className="font-semibold">Pipeline timing · last 30 days</h2><div className="grid gap-3 sm:grid-cols-2">
      <div className="rounded-lg border p-3"><p className="muted">Reading recorded → alert created</p><p className="text-xl font-semibold">{duration(report.alerts.p50_seconds)} median</p><p className="muted">95th percentile {duration(report.alerts.p95_seconds)} · {report.alerts.count} alerts</p></div>
      <div className="rounded-lg border p-3"><p className="muted">Alert created → external delivery</p><p className="text-xl font-semibold">{duration(report.deliveries.p50_seconds)} median</p><p className="muted">95th percentile {duration(report.deliveries.p95_seconds)} · {report.deliveries.count} completed deliveries</p></div>
