...
export default async function MorePage({searchParams}:{searchParams:Promise<{profile?:string}>}) {
  ...
  return <AppFrame profile={view.profile} profiles={profiles}><h1 className="page-title">More</h1><div className="stack">
    <section className="card">{[["Devices & data","/more/data"],["Data freshness & latency","/more/advanced/latency"],["Cycle tracking","/more/cycle"],["Display","/more/display"],["Advanced","/more/advanced"]].map(([name,href])=><a className="list-row" key={href} href={href+suffix}><span>{name}</span></a>)}<a className="list-row" href="/account">Your account</a><a className="list-row" href="/legal/disclaimer">Health disclaimer</a></section>
