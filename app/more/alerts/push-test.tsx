...
export function PushTest() {
  ...
  return <section className="card stack"><h2 className="type-section">Instant browser notifications</h2><p role="status">{status}</p><p className="muted">When HMS receives an unusual live reading, this device can be notified even when the site is closed. Lock-screen text never includes health values.</p><div className="flex flex-wrap gap-2"><button disabled={!ready} onClick={enable} className="button secondary">Enable instant alerts</button><button disabled={!ready} onClick={test} className="button secondary">Test notification</button><button disabled={!ready} onClick={disable} className="button secondary">Disable</button></div></section>;
}
