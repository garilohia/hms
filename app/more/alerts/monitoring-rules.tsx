...
export function MonitoringRules({ profile }: { profile: { id: string; name: string } }) {
  ...
  return <div className="space-y-6">
    <section className="space-y-4 rounded-xl border p-4">
      <div><h2 className="text-lg font-semibold">Shared monitoring thresholds</h2><p className="text-sm">Doctors, family caregivers, and profile owners with alert access can set a reading threshold for {profile.name}. When a new reading stays beyond it for the chosen duration, the profile owner and the person who set the rule are notified through enabled channels.</p></div>
      <form onSubmit={submit} className="space-y-3"><fieldset disabled={busy} className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
        <label>Parameter<select className="mt-1 block w-full rounded border p-2" disabled={Boolean(editing)} value={metric} onChange={event => chooseMetric(event.target.value as MetricType)}>{availableMetrics.map(item => <option key={item} value={item}>{label(item)}</option>)}</select></label>
        <label>Trigger<select className="mt-1 block w-full rounded border p-2" disabled={Boolean(editing)} value={comparator} onChange={event => setComparator(event.target.value as keyof typeof comparisons)}>{Object.entries(comparisons).map(([item, text]) => <option key={item} value={item}>{text}</option>)}</select></label>
        <label>Threshold<input aria-label="Monitoring threshold" className="mt-1 block w-full rounded border p-2" type="number" min={chosen.min} max={chosen.max} step={chosen.step} value={value} onChange={event => setValue(event.target.value)} required /></label>
        <label>Threshold basis<select disabled={metric === "skin_temperature" || Boolean(editing)} className="mt-1 block w-full rounded border p-2" value={basis} onChange={event => { const next = event.target.value as typeof basis; setBasis(next); setValue(next === "baseline_deviation" ? "4" : String(limits[metric]?.suggested ?? "")); }}><option value="absolute">Measured value ({units[metric]})</option><option value="baseline_deviation">Personal baseline (MADs)</option></select></label>
        <label>Sustained for (seconds)<input className="mt-1 block w-full rounded border p-2" type="number" min="0" max="86400" step="1" value={duration} onChange={event => setDuration(event.target.value)} required /></label>
        <label>Priority<select className="mt-1 block w-full rounded border p-2" value={severity} onChange={event => setSeverity(event.target.value as typeof severity)}><option value="info">Information</option><option value="attention">Attention</option><option value="urgent">Urgent</option></select></label>
      </fieldset><p className="text-sm">{chosen.note} {chosen.suggested !== undefined && <>The prefilled value is an illustrative starting point, not a prescription; confirm it with the person’s clinician.</>}</p><div className="flex flex-wrap gap-3"><button disabled={busy} className="rounded-lg border px-4 py-2">{busy ? "Saving…" : editing ? "Save changes" : "Add my rule"}</button>{editing && <button type="button" disabled={busy} onClick={cancelEdit} className="underline">Cancel editing</button>}</div></form>
      <p role="status" className="text-sm">{message}</p>
      <div className="space-y-2">{data.rules.map(rule => <article key={rule.id} className="flex flex-wrap items-center justify-between gap-3 rounded-lg border p-3"><div><p className="font-medium">{label(rule.metric_type)} {comparisons[rule.comparator]} {rule.value} {rule.threshold_type === "absolute" ? units[rule.metric_type] : "MADs from baseline"}</p><p className="text-sm">For {rule.min_duration_s} seconds · {rule.severity} · Set by {rule.author_name} ({rule.author_role})</p></div>{rule.can_edit && <div className="flex gap-3"><button disabled={busy} onClick={() => startEdit(rule)} className="text-sm underline">Edit</button><button disabled={busy} onClick={() => void remove(rule.id)} className="text-sm underline">Remove</button></div>}</article>)}{data.rules.length === 0 && <p className="text-sm">No shared monitoring thresholds yet.</p>}</div>
      <p className="text-sm">HMS processes a reading immediately after receiving it. End-to-end delay also includes the wearable’s device-to-phone-to-provider sync, which HMS cannot control. This is monitoring support, not emergency detection or a substitute for medical care.</p>
    </section>
    <section className="space-y-3 rounded-xl border p-4"><h2 className="text-lg font-semibold">Notification readiness</h2>{data.channels.map(channel => <p key={channel.recipient}><strong>{channel.recipient}{channel.is_current ? " (this account)" : ""}:</strong> Email {channel.email_enabled ? "enabled" : "off"} · Push {channel.push_enabled ? "enabled" : "off"}</p>)}<p className="text-sm">A rule can create an in-app alert even when email and push are off. Enable at least one external channel on each account that should be notified.</p></section>
    <section className="space-y-3 rounded-xl border p-4"><h2 className="text-lg font-semibold">Recent alert delivery activity</h2>{data.activity.map(item => <article key={item.id} className="space-y-2 rounded-lg border p-3"><p className="font-medium">{item.severity} · {item.acknowledged_at ? "Acknowledged" : "Not acknowledged"}{item.is_historical ? " · Historical" : ""}{item.is_sample ? " · Sample data" : ""}</p><p>{item.body || "Reading details unavailable."}</p><p className="text-sm">Fired {new Date(item.fired_at).toLocaleString()}</p>{item.deliveries.length ? <ul className="list-disc pl-5 text-sm">{item.deliveries.map((delivery, index) => <li key={`${delivery.recipient}:${delivery.channel}:${index}`}>{delivery.recipient} · {delivery.channel} · {delivery.status}{delivery.delivered_at ? ` at ${new Date(delivery.delivered_at).toLocaleString()}` : ""}</li>)}</ul> : <p className="text-sm">In-app only; no external delivery was queued.</p>}</article>)}{data.activity.length === 0 && <p className="text-sm">No alert activity yet.</p>}</section>
  </div>;
}
