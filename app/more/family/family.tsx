...
export function Family({profile,profiles,accountCode,initialLinks,initialIncoming}:{profile:PatientProfile;profiles:PatientProfile[];accountCode:string;initialLinks:Links;initialIncoming:Links}) {
  ...
  return <div className="stack"><p role="status">{status}</p>
    ...
    <section className="card stack"><h2>Shared with you</h2>{incoming.rows.length?incoming.rows.map(l=><div key={l.id} className="stack"><h3>{l.name}</h3><p>{l.status}. Scope: {l.granted_scopes.join(", ")}</p>
    ...
    <section className="card stack"><h2>Caregivers for {profile.name}</h2>{links.rows.length?links.rows.map(l=><div key={l.id} className="stack"><h3>{l.name}</h3><p>{l.role}, {l.status}. Scope: {l.granted_scopes.join(", ")}</p>
