...
export function DataImport({ profiles, onBusyChange, onComplete }: { profiles: Profile[]; onBusyChange?: (busy:boolean)=>void; onComplete?:()=>void }) {
  ...
  return <section className="space-y-5">
    ...
    <p className="text-sm">Folders are read recursively on this device. HMS recognises its four-column template and Google Fit Daily activity metrics columns for steps, calories, average heart rate and average weight. Other CSV formats are named and skipped. Use the same source name when re-importing. <a className="underline" href="/api/ingestion/template">Download CSV template</a>.</p>
    ...
    {progress && <div role="status" data-testid="import-progress" data-status={progress.status} data-inserted={progress.inserted} data-skipped={progress.skipped} data-file-count={progress.fileCount} data-skipped-files={progress.skippedFiles} className="rounded border p-4">
      ...
      {progress.fileCount && <p>{progress.fileName ? progress.fileName + " · " : ""}{progress.fileIndex?.toLocaleString()} of {progress.fileCount.toLocaleString()} files. {(progress.skippedFiles || 0).toLocaleString()} files skipped.</p>}
