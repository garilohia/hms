...
export default async function DataPage({searchParams}:{searchParams:Promise<{profile?:string;integration?:string}>}) {
  ...
  const notice=query.integration==="connected"?"Wearable account connected. The first sync is queued. You can also use Sync latest 7 days below.":query.integration==="synced"?"Wearable account connected and the latest seven days were synced.":query.integration==="connected_sync_failed"?"Wearable account connected, but the first sync could not finish. Use Sync latest 7 days below to retry.":query.integration==="setup"?"This connector needs its provider credentials and token-encryption key before it can be linked.":query.integration==="failed"?"The wearable connection was not completed. Please retry.":query.integration==="denied"?"This profile cannot be linked without ownership and processing consent.":"";
