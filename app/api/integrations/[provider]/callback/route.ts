...
export async function GET(request: Request, context: {params: Promise<{provider: string}>}) {
  ...
  try {
    ...
    await saveIntegration(session.user.id, state.subject, provider.data, () => exchangeCode(provider.data, code, callbackUrl(url.origin, provider.data),state.verifier,state.nonce));
    return Response.redirect(new URL(`/more/data?profile=${state.subject}&integration=connected`, request.url));
  } catch {
