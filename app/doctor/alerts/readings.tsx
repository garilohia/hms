...
export function SharedAlerts({initial}:{initial:PatientView}) {
  ...
  return <div className="stack"><p className="muted">Read-only. Only the patient or owning guardian can acknowledge readings.</p>
    {view.alerts?.length?view.alerts.map(a=><section className="card stack" key={a.id}>{a.is_sample&&<span className="sample-badge">Sample data</span>}<h2>{a.severity} reading</h2><p>{a.metric_snapshot.body}</p><p className="muted">{a.acknowledged_at?"Acknowledged":"Not acknowledged"}</p></section>):<p>No recorded alerts.</p>}
