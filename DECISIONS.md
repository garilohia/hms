...
## D041 — Fair provider pages and persisted connection cooldowns

Version 2 of the existing private JSON checkpoint stores a cursor/completion flag per collection and rotates reconciliation between collections. A dense heart-rate history can no longer prevent the first SpO₂, temperature or sleep page from being visited. Legacy checkpoints remain readable and are upgraded by compare-and-swap before fetching; token generation, consent, ingestion deduplication and the frozen-window completion watermark stay unchanged. No database migration is needed for the versioned JSON shape.

For Google Health only, an older, still-paginated sweep alternates reconciliation with a rotating current head-page probe. Google's list contract documents descending interval start time. Head probes can bring newer observations forward but never replace a reconciliation token, mark a collection complete or advance `last_sync_at`. WHOOP retains fair reconciliation without claiming a current-head ordering contract. Scheduled work yields after at most three pages or its eight-second phase-start budget, within the shared tick budget; the existing five-second persistence reserve remains. These bounds do not guarantee completion latency or capacity for any number of users.

A provider HTTP 429 stores a connection-scoped cooldown against the current grant, using `Retry-After`, WHOOP reset seconds where applicable, or a five-minute fallback. Valid long delays are not shortened. Every subsequent page, manual sync and token refresh checks the cooldown; completion and scheduler cleanup preserve a concurrently established future cooldown. Manual sync returns a redacted HTTP 429 with a rounded-up `Retry-After`. This is not an aggregate client-quota limiter: approved provider capacity, a shared budget and real account/device acceptance remain required before claiming minute-level service. Sources: [Google list ordering](https://developers.google.com/health/reference/rest/v4/users.dataTypes.dataPoints/list), [WHOOP quotas](https://developer.whoop.com/docs/developing/rate-limiting/), [HTTP retry dates](https://www.rfc-editor.org/rfc/rfc9110.html#name-retry-after).

Review corrections before release: the eight-second slice bounds page starts, not an already-started fetch. That fetch can use the remaining shared deadline less the persistence reserve; otherwise a healthy four-second response could be aborted forever. A tail connection that advances no checkpoint stays ahead of serviced work on the next tick. A refresh-endpoint cooldown is committed while the original grant lock is still held, then reported outside the transaction; a waiting refresh cannot rotate the grant between a rollback and a separate cooldown write. Deterministic slow-page, tail-priority and concurrent-refresh tests cover these cases.

## D042 — Coalesced live reads and truthful recovery state

Patient invalidations use one active read and one coalesced trailing read. More broadcasts do not cancel an in-flight response or keep postponing its start. Transient read failures retry at 500, 1,500 and 4,000 milliseconds without needing another ping; reads have a 15-second abort deadline. Repeated pings cannot reset an exhausted retry cycle. Browser offline/online transitions pause/resume work; disposal prevents late responses from overwriting the next view. Authentication, permission and other non-transient failures remain terminal until remount, with their safe server message preserved. No automatic retry is added to a side-effecting command.

Today versions its history cache by subject, date range and current summary. Older chart rows cannot supply the headline figure after a fresher summary arrives; a failed history read is visible and falls back to that summary. Summary-drain status is separate from acknowledgement status, so a later `pending_jobs=0` clears the old drain failure without hiding an acknowledgement error. The live golden test injects one failed History read after the final write, then requires recovery under the original assertion deadline with no extra write or ping. These concrete recovery fixes do not establish the cause of every earlier hosted timeout (OQ016).

Profile-keyed Today, History and consultation-note components discard the previous subject's local state during navigation. History stops and awaits its old live read before a manual range request; an unmounted request cannot replace the next subject's view. Read failures show truthful recovery status in all three display modes. Deployment source excludes local credentials and browser-test artefacts explicitly in `.vercelignore`; a dry run must confirm these exclusions before a CLI upload.

## D043 — Shared provider request allowance before outbound HTTP

Use one private, forced-RLS Postgres token bucket per WHOOP client or Google project. Store only a provider name, SHA-256 scope digest, credits, refill time and cooldown; no patient, account, token or health values. Server-only INVOKER functions have no PUBLIC, browser-role or service-role execute/table grants. Existing server database access reserves a permit in an independent autocommit call, closes that connection, then contacts the provider. A later grant-transaction rollback, timeout or uncertain response never refunds the permit. No quota lock is held across HTTP.

The fixed HMS starting caps are conservative: WHOOP replenishes 9,000 permits per day with a burst of ten; Google uses one permit per second with a burst of one **across the whole project**, deliberately below its per-user allowance without storing user identity. These are safety limits, not approved quotas or a capacity promise. WHOOP's upper token-bucket envelope is 9,010 requests in any 24 hours. The database clock is read after locking; a backwards clock cannot refill credits. Concurrent 429 cooldowns retain the latest retry time. All provider HTTP attempts, including OAuth, identity, refresh and revocation, share the wrapper. Local exhaustion is queued separately from a provider error and does not advance a checkpoint.

Require an explicit canonical `GOOGLE_HEALTH_QUOTA_PROJECT_ID`; WHOOP uses its configured client ID. Do not infer a Google project from an OAuth client string. Deployments using the same provider scope must share the budget database; an isolated Preview must use separate provider credentials/projects. Missing configuration/migration or an unavailable budget store prevents outbound requests. Credentials used by other software are outside HMS's accounting, so launch requires checking the actual allocation and measuring queue fairness/source delay. Local disconnect remains immediate even when remote revocation must wait.

Sources checked 19 September 2026: [WHOOP limits](https://developer.whoop.com/docs/developing/rate-limiting/), [Google Health limits](https://developers.google.com/health/rate-limits), [Supabase RLS and grants](https://supabase.com/docs/guides/database/postgres/row-level-security). This repository uses Drizzle migrations; generated custom migration 0042 follows that existing workflow rather than introducing a second migration ledger.

OAuth failure boundary: WHOOP issues tokens before its separate identity lookup. A failed or quota-denied identity request can therefore leave permission at the provider without a completed HMS connection, as can other failures after token exchange. Do not save an unverified identity, retry a single-use code or claim that remote permission was removed. The failed-link notice explicitly directs the owner to remove HMS in the provider's app-permission settings before trying again. Tests cover second-request denial; authorised provider acceptance must verify this manual recovery. Automatically retaining/revoking an identity-less pending grant is not implemented or claimed.

The request wrapper retains its validated URL clone across awaited budget/authority work, so a caller cannot mutate a URL object to change the destination after validation. The regression covers that mutation; immutable string callers are unchanged.
