...
- Choose the history timezone before importing; post-import re-bucketing is not implemented (OQ005). The browser must remain open, awake and online for a large import. Safe re-import requires the same source name. No offline health-data cache or physical-phone verification is claimed.
...
6. **Configure only the real notification services being launched.** For email, supply a verified Resend sender, `RESEND_API_KEY` and `EMAIL_FROM`; test only authorised recipients and monitor failed/exhausted delivery rows. Sample data remains stubbed. Web Push requires implementing persistent subscriptions, recipient scoping and transport, not merely adding VAPID fields.
...
8. **Review product limits before advertising them.** Refresh dated catalogue sources, add/verify any additional PDF script fonts and decide whether timezone re-bucketing, realtime chat, paid/video consultations or future device integrations are required. None should be represented as already shipped.
